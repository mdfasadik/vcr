...
export default function EditNoiceForm({ defaultValue, showDeleteModal }) {
  const [category, setCategory] = useState(defaultValue.category);
  const [selectedSec, setSelectedSec] = useState(defaultValue.section);
  const [showLink, setShowLink] = useState(
    defaultValue.supportingLinks.length > 0 ? true : false
  );
  const [supportingLinks, setSupportingLinks] = useState(
    defaultValue.supportingLinks
  );
  ...
  const {
    ...
    defaultValues: {
      ...
      postedBy: defaultValue.postedBy,
    },
  });
  ...
  const onSubmit = async (data) => {
    ...
    supportingLinks.forEach((link) => delete link._id);
    const payload = { ...data, category, supportingLinks };

    setLoading(true);
    const res = await http.put(
      process.env.NEXT_PUBLIC_API + "notice/" + defaultValue._id,
      payload
    );
    setLoading(false);
    if (res.data) {
  ...
  return (
    <>
      <div className='text-sm font-medium mb-8 flex gap-2 cursor-pointer'>
        ...
        <p className='text-center'>Update notice</p>
      </div>
      ...
      <button
        ...
        className='btn mt-4 w-full bg-red-600'>
        Delete Notice
      </button>
    </>
  );
